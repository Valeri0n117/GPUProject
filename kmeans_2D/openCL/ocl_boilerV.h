...
cl_platform_id select_platform() {
	...
	cl_uint nump = 0;  // on this pc 0_OCL_CPU 1GPU 2ExCPU 3MAD_CPU

	
	/* OLD ASK
	int x=1;
	printf("ID piattaforma: ");
	scanf("%d", &x); */
...
cl_device_id select_device(cl_platform_id p)
{
	...
	do{
		...
		//if only 1 device, no choice needed
		if(ndevs==1){
			numd=0;
		}else{
			int x=0;
